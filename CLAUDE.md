...
`/api/v1/albums` — **동화 앨범.** 여기만 다르다. 중계가 아니라 이 서버가 직접 가진
데이터이고, MySQL 에 남긴다(`com.zooearly.album`). 2026-08-27 에 제품 결정으로 추가됐다.

## 절대 하지 말 것 ★

이 프로젝트에서 가장 중요한 제약이다. 아래를 어기면 설계가 무너진다.

- **중계 경로(`/api/v1/ai/*`)에 DB를 끌어들이지 않는다.** 그 경로는 무상태로 둔다.
  DB를 쓰는 것은 앨범(`com.zooearly.album`) 하나뿐이고, 중계와 섞지 않는다
- **중계 경로에 비즈니스 로직을 넣지 않는다.** 대화 이력 관리, 사용자 정보 조회, 진행 상태 저장 — 전부 앱 또는 FastAPI의 일이다
- **FastAPI 응답 body를 파싱·가공하지 않는다.** String으로 받아 그대로 통과시킨다. 파싱하는 순간 FastAPI 스키마가 바뀔 때마다 이 서버도 재배포해야 한다.
  앨범을 만들 때 이 규칙을 지키려고 **동화 생성 응답을 가로채지 않았다** — 앱이 받은 동화를 앨범 API로 다시 올린다
- **인증·로그인을 추가하지 않는다.** 앨범은 기기가 만든 UUID(`childId`)로 묶고, 조회할 때도 그 값을 반드시 요구한다.
  번호만으로 남의 동화를 열 수 없어야 한다
- **엔드포인트를 임의로 추가하지 않는다.** 필요해 보이면 먼저 사람에게 확인한다.
  `/api/v1/albums`는 2026-08-27에 확인을 거쳐 추가됐다
...
| `feature/작업내용` | 작업용 임시 | `dev`에서 따고 `dev`로 PR. **머지되면 즉시 삭제한다** |

흐름: `dev`에서 `feature/*` 생성 → PR → `dev` 머지 → 브랜치 삭제 → 발표·배포 시점에 `dev` → `main` PR

`main`을 비워두지 않는다. 항상 "그 시점에 돌아가는 버전"이어야 발표 직전 안전판이 된다.
