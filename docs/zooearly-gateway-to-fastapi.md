...
## 1-1. ✅ 해소됨 (2026-08-25) — 아래는 그때 무엇이 어긋났는지의 기록이다

**추론 서버가 응답·요청을 모두 앱 계약에 맞췄다.** 7개 경로가 전부 붙는다.
확인은 `node scripts/smoke-test.mjs` (저장소 루트) 로 한 번에 할 수 있다.

| 항목 | 어떻게 해결했나 |
|---|---|
| 성공/에러 봉투 | 추론 서버가 `{success, data}` / `{success, error}` 를 직접 만든다 |
| 필드 표기 | 응답을 camelCase 로 직렬화한다 |
| 언어 코드 | 응답은 `KOREAN`/`VIETNAMESE`/`CHINESE` 로 올려 보낸다 |
| TTS 응답 | base64 JSON (`data.audio.data` + `format`) |
| 요청 파트 이름 | `audio`·`sentenceId`·`childName` 을 받는다. 기존 snake_case 도 함께 받는다 |
| 없던 경로 | `/story/generate`, `/chat`, `/feedback/expression` 을 추가했다 |

**게이트웨이는 고치지 않았다.** `application.yml` 의 경로 설정도 그대로다 —
추론 서버가 게이트웨이가 이미 보내던 형태를 받아주는 쪽으로 맞췄기 때문이다.

`/feedback/expression`(표현 교정)은 "당분간 만들지 않는다"였는데 만들었다. 화면
③④("이렇게 말하면 더 자연스러워요" · "이 말의 뜻이에요")를 구현하면서 필요해졌고,
번역까지 한 응답에 담아 앱이 `/translate` 를 따로 부르지 않아도 되게 했다.

---

## 1-1(과거). ★ 지금 맞춰야 할 것 — before / after
...
| 엔드포인트 | 제한 | 왜 |
|---|---|---|
| `/ai/pronunciation` | **65초** | 채점 서비스 콜드 스타트(실측 38초) + FastAPI 재시도(최대 62초) |
| `/ai/story` | **60초** | 4장면을 한 번에 생성한다 (실측 14~18초) |
| `/ai/stt` | **45초** | STT 콜드 스타트 실측 43초 |
| `/ai/chat` | **30초** | STT + LLM + TTS 3단 |
| `/ai/tts` `/ai/feedback` `/ai/pronunciation/sentences` | **15초** | 예열 상태 실측 1~6초 |
| 연결(TCP) | 3초 | |

**긴 값들은 모델이 느려서가 아니라 컨테이너가 자고 있어서다.** 발음 채점과 STT 는
min-replicas 가 0인 Azure Container App 을 부른다. 유휴 뒤 첫 요청이 컨테이너를 깨우는
동안 30~43초가 걸리고(2026-08-25 실측), 그 뒤로는 1초 안에 끝난다.

발음 채점은 그 기다림이 **아이가 마이크를 누른 뒤에** 오지 않도록, FastAPI 가 문장 목록을
줄 때 채점 서비스를 미리 깨워둔다. 그래서 65초까지 가는 일은 예열이 늦은 날뿐이다.

`/ai/story`의 60초는 실측(14~18초)보다 넉넉하다. LLM 지연 편차를 감안한 값이다.
