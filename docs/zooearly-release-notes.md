...
## 2026-08-25 · 추론 서버 연동 해소 · 프론트엔드 구현 (미배포)

**게이트웨이의 릴레이·검증 코드는 바뀌지 않았다.** 어긋나 있던 쪽(추론 서버)이
게이트웨이가 이미 보내던 형태를 받아주도록 맞췄다. `application.yml` 의 경로 설정도 그대로다.

바뀐 것은 `settings.gradle` 하나다 — foojay 툴체인 리졸버를 넣어, JDK 17 이 깔려 있지
않은 PC 에서도 `./gradlew build` 가 17 을 자동으로 받아 쓰게 했다. `build.gradle` 의
toolchain 은 17 그대로라 CI(`actions/setup-java` 17)·Dockerfile(`gradle:8.10-jdk17`)과
어긋나지 않는다. toolchain 을 21 로 올려 해결했다면 그 셋이 갈라졌을 것이다.

- 7개 경로가 전부 붙는다. 그전에는 문장 목록 하나를 빼고 전부 `422`/`502` 였다
  (연동 문서 §1-1 실측표)
- 추론 서버가 `{success, data}` 봉투와 camelCase 를 직접 만든다. 게이트웨이가 body 를
  파싱하지 않는 구조라(§0.1) 그쪽이 만들 수밖에 없었다
- TTS 가 base64 JSON 으로 바뀌었다. 바이너리는 `ResponseEntity<String>` 릴레이에서 깨진다
- 요청 파트 이름을 추론 서버가 두 벌(`audio`/`audio_file`, `sentenceId`/`sentence_id`)
  받는다. 게이트웨이 변환 코드를 넣지 않아도 되게 하려는 것이다
- `/feedback`(표현 교정)이 **살아났다.** "당분간 만들지 않는다"였는데 화면 ③④를
  구현하면서 필요해졌고, 모국어 번역까지 한 응답에 담는다.
  게이트웨이 쪽 검증 코드는 이미 있던 것을 그대로 쓴다
- `INFERENCE_TIMEOUT_STORY` 는 그대로 60초다. 목 응답 기준으로는 즉시라 실측값을
  아직 못 얻었다 — `PROVIDER=real` 로 돌려본 뒤 줄인다

**프론트엔드가 새로 붙었다** (저장소 루트 `frontend/`, Vite + React).
Expo 프로토타입을 대체한다. 브라우저에서 도는 정적 앱이라 `CORS_ALLOWED_ORIGINS` 가
실서버에서도 필수다 — 비어 있으면 게이트웨이가 CORS 를 아예 켜지 않아 모든 호출이 막힌다.

확인: 저장소 루트에서 `node scripts/smoke-test.mjs [게이트웨이 주소]`

---

## 2026-08-24 · 동화 생성 추가 (미배포)

`POST /api/v1/ai/story` 신설 — 하루치 플레이 기록 4장면을 LLM이 동화로 엮는다.
앱 명세 **v1.7.0**.

- **서버는 기록을 저장하지 않는다.** 앱이 하루를 진행하며 4장면을 쌓아뒀다가 한 번에 보낸다 (무상태 유지)
- **타임아웃만 60초로 따로 뒀다.** 다른 엔드포인트는 15초인데, 4장면을 한 번에 생성해서다.
...
- 요청 필드는 **camelCase로 나간다**(`childName`, `partnerLine` …). 게이트웨이가 앱 body를
  가공 없이 통과시키기 때문이다.
  ✅ **2026-08-25 해소** — 추론 서버가 camelCase 를 받는다(snake_case 도 함께 받는다).
  게이트웨이에 변환 코드를 넣지 않아도 됐다
